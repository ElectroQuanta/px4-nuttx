/****************************************************************************
 * arch/arm/src/mx8mn/mx8mn_clockconfig.c
 *
...
#include "mx8mn_ccm.h"
#include "mx8mn_clockconfig.h"
#include "hardware/mx8mn_gpc.h"
...
void mx8mn_clockconfig(void)
{
  /* Ensure the clock of the peripherals used by M core not be affected by
   * A core which is running in a different domain.
   */

  mx8mn_ccm_gate_clock(CCM_IOMUX_CLK_GATE,  CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_IPMUX1_CLK_GATE, CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_IPMUX2_CLK_GATE, CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_IPMUX3_CLK_GATE, CLK_ALWAYS_NEEDED);

  mx8mn_ccm_gate_pll(SYSTEM_PLL1_CLK, CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_pll(SYSTEM_PLL2_CLK, CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_pll(SYSTEM_PLL3_CLK, CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_pll(AUDIO_PLL1_CLK,  CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_pll(AUDIO_PLL2_CLK,  CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_pll(VIDEO_PLL_CLK,   CLK_ALWAYS_NEEDED);

  /* Set Cortex-M7 clock source to System PLL1 with no division (800MHz) */

  mx8mn_ccm_configure_clock(ARM_M7_CLK_ROOT, SYSTEM_PLL1_CLK, 1, 1);

  /* Add dependency on OCRAM and RDC module to the M7 */

  mx8mn_ccm_gate_clock(CCM_OCRAM_CLK_GATE,  CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_RDC_CLK_GATE,    CLK_ALWAYS_NEEDED);
  ...
  mx8mn_ccm_gate_clock(CCM_OCRAM_CLK_GATE,      CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_RDC_CLK_GATE,        CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_SIM_M_CLK_GATE,      CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_SIM_MAIN_CLK_GATE,   CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_SIM_S_CLK_GATE,      CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_SIM_WAKEUP_CLK_GATE, CLK_ALWAYS_NEEDED);
  mx8mn_ccm_gate_clock(CCM_DEBUG_CLK_GATE,      CLK_ALWAYS_NEEDED);
  ...
  mx8mn_ccm_enable_clock(IPG_CLK_ROOT);
  mx8mn_ccm_enable_clock(AHB_CLK_ROOT);
  mx8mn_ccm_enable_clock(MAIN_AXI_CLK_ROOT);
  mx8mn_ccm_enable_clock(DRAM_ALT_CLK_ROOT);
...
#  ifdef CONFIG_MX8MN_I2C1
  ...
  mx8mn_ccm_enable_clock(I2C1_CLK_ROOT);
#  endif
#  ifdef CONFIG_MX8MN_I2C2
  ...
  mx8mn_ccm_enable_clock(I2C2_CLK_ROOT);
#  endif
#  ifdef CONFIG_MX8MN_I2C3
  ...
  mx8mn_ccm_enable_clock(I2C3_CLK_ROOT);
#  endif
#  ifdef CONFIG_MX8MN_I2C4
  ...
  mx8mn_ccm_enable_clock(I2C4_CLK_ROOT);
#  endif
#endif  

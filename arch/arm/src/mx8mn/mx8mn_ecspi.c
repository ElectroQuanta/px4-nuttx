/******************************************************************************
 * arch/arm/src/mx8mn/mx8mn_ecspi.c
 *
...
#include "hardware/mx8mn_ecspi.h"
...
static void mx8mn_spi_enable(struct mx8mn_spi_s *dev);
static int mx8mn_spi_interrupt(int irq, void *context, void *arg);
static int mx8mn_spi_transfer(struct mx8mn_spi_s *priv,
...
static void        spi_bus_initialize(struct mx8mn_spi_s *priv);
...
#ifdef CONFIG_MX8MN_SPI1
static const struct spi_ops_s g_spi1_ops =
{
  .lock         = spi_lock,
  .select       = mx8mn_spi1_select,        /* Provided externally by board logic */
...
#endif
  .status       = mx8mn_spi1_status,        /* Provided externally by board logic */
#ifdef CONFIG_SPI_CMDDATA
  .cmddata      = mx8mn_spi1_cmddata,
#endif
  .send         = spi_send,
...
static struct mx8mn_spi_s g_spi1_dev =
{
  ...
  .irq        = MX8MN_IRQ_ECSPI1,
  .lock       = NXMUTEX_INITIALIZER,
  .wait       = SEM_INITIALIZER(0),
};
#endif

#ifdef CONFIG_MX8MN_SPI2
static const struct spi_ops_s g_spi2_ops =
{
  .lock         = spi_lock,
  .select       = mx8mn_spi2_select,        /* Provided externally by board logic */
  .setfrequency = spi_setfrequency,
  .setmode      = spi_setmode,
  .setbits      = spi_setbits,
#ifdef CONFIG_SPI_HWFEATURES
  .hwfeatures   = 0,                        /* Not supported */
#endif
  .status       = mx8mn_spi2_status,        /* Provided externally by board logic */
#ifdef CONFIG_SPI_CMDDATA
  .cmddata      = mx8mn_spi2_cmddata,
#endif
  .send         = spi_send,
#ifdef CONFIG_SPI_EXCHANGE
  .exchange     = spi_exchange,
...
static struct mx8mn_spi_s g_spi1_dev =
{
  ...
  .irq        = MX8MN_IRQ_ECSPI2,
  .lock       = NXMUTEX_INITIALIZER,
  .wait       = SEM_INITIALIZER(0),
};
#endif

#ifdef CONFIG_MX8MN_SPI3
static const struct spi_ops_s g_spi3_ops =
{
  .lock         = spi_lock,
  .select       = mx8mn_spi3_select,        /* Provided externally by board logic */
  .setfrequency = spi_setfrequency,
  .setmode      = spi_setmode,
  .setbits      = spi_setbits,
#ifdef CONFIG_SPI_HWFEATURES
  .hwfeatures   = 0,                        /* Not supported */
#endif
  .status       = mx8mn_spi3_status,        /* Provided externally by board logic */
#ifdef CONFIG_SPI_CMDDATA
  .cmddata      = mx8mn_spi3_cmddata,
#endif
  .send         = spi_send,
#ifdef CONFIG_SPI_EXCHANGE
  .exchange     = spi_exchange,
...
static struct mx8mn_spi_s g_spi3_dev =
{
  .dev.ops    = &g_spi3_ops,
  .base       = MX8M_ECSPI3,
  .clock      = ECSPI3_CLK_ROOT,
  .irq        = MX8MN_IRQ_ECSPI3,
  .lock       = NXMUTEX_INITIALIZER,
  .wait       = SEM_INITIALIZER(0),
};
#endif
...
static void mx8mn_spi_enable(struct mx8mn_spi_s *priv)
{
...
static inline int mx8mn_spi_wait_irq(struct mx8mn_spi_s *priv, int nsec)
{
  return nxsem_tickwait(&priv->wait, NSEC2TICK(nsec));
}

/******************************************************************************
 * Name: mx8mn_spi_interrupt
 *
 * Description:
...
static int mx8mn_spi_interrupt(int irq, void *context, void *arg)
{
  struct mx8mn_spi_s *priv = (struct mx8mn_spi_s *)arg;
                              ...
                              const uint8_t *out, uint8_t *in, int size)
{
  ...
  int ret = mx8mn_spi_wait_irq(priv, priv->byte_duration * (size + 1)); /* add one byte duration of leeway */
  if (ret < 0)
    {
      return ret;
    }
...
static int spi_lock(struct spi_dev_s *dev, bool lock)
{
  struct mx8mn_spi_s *priv = (struct mx8mn_spi_s *)dev;
  int ret;
...
static uint32_t spi_setfrequency(struct spi_dev_s *dev, uint32_t frequency)
{
  ...
  uint32_t clock = mx8mn_ccm_get_clock(priv->clock);
...
static uint32_t spi_send(struct spi_dev_s *dev, uint32_t wd)
{
  ...
  uint32_t dummy;
  ret = mx8mn_spi_transfer(priv, (const uint8_t *)&wd,
                                 (uint8_t *)&dummy,
                                 priv->nbits % 8);
                         ...
                         void *rxbuffer, size_t nwords)
{
  ...
  while (transferred < size)
    {
      int ret = mx8mn_spi_transfer(priv,
...
static void spi_bus_initialize(struct mx8mn_spi_s *priv)
{
  ...
  irq_attach(priv->irq, mx8mn_spi_interrupt, priv);
...
struct spi_dev_s *mx8mn_spibus_initialize(int bus)
{
  struct mx8mn_spi_s *priv = NULL;

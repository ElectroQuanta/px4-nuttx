/****************************************************************************
 * arch/arm/src/mx8mn/mx8mn_i2c.c
 *
...
static bool is_busy(struct mx8mn_i2c_s *priv);
static void mx8mn_i2c_enable(struct mx8mn_i2c_s *priv);
static void mx8mn_i2c_disable(struct mx8mn_i2c_s *priv);
static int  mx8mn_i2c_interrupt(int irq, void *context, void *arg);
static void mx8mn_i2c_reset_bus(struct mx8mn_i2c_s *dev);

/* I2C lower half driver methods */

static int  mx8mn_i2c_transfer(struct i2c_master_s *dev,
                               struct i2c_msg_s *msgs, int count);
static int  mx8mn_i2c_reset(struct i2c_master_s *dev);
...
static const struct i2c_ops_s mx8mn_i2c_ops =
{
  .transfer = mx8mn_i2c_transfer
#ifdef CONFIG_I2C_RESET
  , .reset  = mx8mn_i2c_reset
#endif
};
...
static struct mx8mn_i2c_s g_i2c1_dev =
{
  .dev.ops    = &mx8mn_i2c_ops,
  .base       = MX8M_I2C1,
  .clock      = I2C1_CLK_ROOT,
  .irq        = MX8MN_IRQ_I2C1,
  .refs       = 0,
...
static struct mx8mn_i2c_s g_i2c2_dev =
{
  .dev.ops    = &mx8mn_i2c_ops,
  .base       = MX8M_I2C2,
  .clock      = I2C2_CLK_ROOT,
...
static struct mx8mn_i2c_s g_i2c3_dev =
{
  .dev.ops    = &mx8mn_i2c_ops,
  .base       = MX8M_I2C3,
  .clock      = I2C3_CLK_ROOT,
...
static struct mx8mn_i2c_s g_i2c4_dev =
{
  .dev.ops    = &mx8mn_i2c_ops,
  .base       = MX8M_I2C4,
  .clock      = I2C4_CLK_ROOT,
...
#ifdef CONFIG_MX8MN_I2C5
static struct mx8mn_i2c_s g_i2c5_dev =
{
  .dev.ops    = &mx8mn_i2c_ops,
  .base       = MX8M_I2C5,
  .clock      = I2C5_CLK_ROOT,
...
#ifdef CONFIG_MX8MN_I2C6
static struct mx8mn_i2c_s g_i2c6_dev =
{
  .dev.ops    = &mx8mn_i2c_ops,
  .base       = MX8M_I2C6,
  .clock      = I2C6_CLK_ROOT,
...
static int mx8mn_i2c_init(struct mx8mn_i2c_s *priv)
{
  /* enable peripheral */

  mx8mn_i2c_enable(priv);

  /* Attach Interrupt Handler */

  irq_attach(priv->irq, mx8mn_i2c_interrupt, priv);
...
static int mx8mn_i2c_deinit(struct mx8mn_i2c_s *priv)
{
  ...
  mx8mn_i2c_disable(priv);

  return 0;
}

/****************************************************************************
 * Name: mx8mn_i2c_enable
 *
 * Description:
...
static inline int mx8mn_i2c_wait_irq(struct mx8mn_i2c_s *priv, int usec)
{
  return nxsem_tickwait(&priv->wait, USEC2TICK(usec));
}

/****************************************************************************
 * Name: mx8mn_i2c_send_start
 *
 * Description:
...
static int mx8mn_i2c_send_start(struct mx8mn_i2c_s *priv)
{
  if (is_busy(priv))
    {
      return -EBUSY;
...
/****************************************************************************
 * Name: mx8mn_i2c_send_repeat_start
 *
 * Description:
...
static int mx8mn_i2c_send_repeat_start(struct mx8mn_i2c_s *priv)
{
...
static void mx8mn_i2c_send_stop(struct mx8mn_i2c_s *priv)
{
  modreg16(0, I2CR_MSTA | I2CR_MTX | I2CR_TXAK, priv->base + I2CR_OFFSET);
}

/****************************************************************************
 * Name: mx8mn_i2c_write_byte
 *
 * Description:
...
static int mx8mn_i2c_write_byte(struct mx8mn_i2c_s *priv, uint8_t byte)
{
  int ret;
  uint16_t status;
  ...
  ret = mx8mn_i2c_wait_irq(priv, 500);
  if (ret < 0)
    {
      return ret;
    }
...
static int mx8mn_i2c_write_bytes(struct mx8mn_i2c_s *priv,
                                 const uint8_t *data,
                                 uint32_t size,
                                 bool is_last)
{
  ...
  for (i = 0; i < size; ++i)
    {
      ret = mx8mn_i2c_write_byte(priv, data[i]);
      if (ret < 0)
        {
          return ret;
        }
...
int mx8mn_i2c_read_bytes(struct mx8mn_i2c_s *priv,
                         uint8_t *data,
                         uint32_t size,
                         bool is_last)
{
...
static void mx8mn_i2c_set_frequency(struct mx8mn_i2c_s *priv,
                                    uint32_t frequency)
{
  uint8_t i;
  uint8_t divider;
  ...
  clock = mx8mn_ccm_get_clock(priv->clock);
...
static void mx8mn_i2c_reset_bus(struct mx8mn_i2c_s *priv)
{
  mx8mn_i2c_disable(priv);
  nxsig_usleep(50);
  mx8mn_i2c_enable(priv);
...
static int mx8mn_i2c_interrupt(int irq, void *context, void *arg)
{
  struct mx8mn_i2c_s *priv = (struct mx8mn_i2c_s *)arg;
  modreg16(0, I2SR_IIF, priv->base + I2SR_OFFSET);
...
static int mx8mn_i2c_transfer(struct i2c_master_s *dev,
                              struct i2c_msg_s *msgs, int count)
{
  struct mx8mn_i2c_s *priv = (struct mx8mn_i2c_s *)dev;
  ...
  mx8mn_i2c_set_frequency(priv, msgs->frequency);

  /* Process every message */

  if ((I2C_M_NOSTART & msgs->flags) == 0)
    {
      ret = mx8mn_i2c_send_start(priv);
      if (ret < 0)
        {
          goto error;
        }
  ...
  for (i = 0; i < count; ++i)
    {
      ...
      if ((I2C_M_NOSTART & msg->flags) == 0)
        {
          if (I2C_M_READ & msg->flags)
            {
              ret = mx8mn_i2c_write_byte(priv, I2C_READADDR8(msg->addr));
            }
          else
            {
              ret = mx8mn_i2c_write_byte(priv, I2C_WRITEADDR8(msg->addr));
            }
        }
      ...
      if (ret == 0)
        {
          if (I2C_M_READ & msg->flags)
            {
              ret = mx8mn_i2c_read_bytes(priv,
                msg->buffer, msg->length, last);
            }
          else
            {
              ret = mx8mn_i2c_write_bytes(priv,
                msg->buffer, msg->length, last);
            }
        }
      ...
      if (!last && ((I2C_M_NOSTART & msg->flags) == 0))
        {
          ret = mx8mn_i2c_send_repeat_start(priv);
          if (ret < 0)
            {
              goto error;
            }
...
struct i2c_master_s *mx8mn_i2cbus_initialize(int port)
{
  struct mx8mn_i2c_s *priv;
...
/****************************************************************************
 * Name: mx8mn_i2cbus_uninitialize
 *
 * Description:
...
int mx8mn_i2cbus_uninitialize(struct i2c_master_s *dev)
{
  struct mx8mn_i2c_s *priv = (struct mx8mn_i2c_s *)dev;

/****************************************************************************
 * arch/arm/src/mx8mn/mx8mn_irq.c
 *
...
/* Vector table base address: defined in vectors.S / linker script */
...
static void mx8mn_dump_nvic(const char *msg, int irq)
{
  irqstate_t flags;
...
#else
#  define mx8mn_dump_nvic(msg, irq)
#endif

/****************************************************************************
 * Name: mx8mn_nmi, mx8mn_pendsv,
 *       mx8mn_dbgmonitor, mx8mn_pendsv, mx8mn_reserved
 *
 * Description:
...
static int mx8mn_nmi(int irq, void *context, void *arg)
{
  up_irq_save();
...
static int mx8mn_pendsv(int irq, void *context, void *arg)
{
  up_irq_save();
...
static int mx8mn_dbgmonitor(int irq, void *context, void *arg)
{
  up_irq_save();
...
static int mx8mn_reserved(int irq, void *context, void *arg)
{
  up_irq_save();
...
static inline void mx8mn_prioritize_syscall(int priority)
{
  uint32_t regval;
...
static int mx8mn_irqinfo(int irq, uintptr_t *regaddr, uint32_t *bit,
                         uintptr_t offset)
{
  DEBUGASSERT(irq >= MX8MN_IRQ_NMI && irq < NR_IRQS);

  /* Check for external interrupt */

  if (irq >= MX8MN_IRQ_FIRST)
    {
      if (irq < (MX8MN_IRQ_FIRST + 32))
        {
           *regaddr = (NVIC_IRQ0_31_ENABLE + offset);
           *bit     = 1 << (irq - MX8MN_IRQ_FIRST);
        }
      else if (irq < (MX8MN_IRQ_FIRST + 64))
        {
           *regaddr = (NVIC_IRQ32_63_ENABLE + offset);
           *bit     = 1 << (irq - MX8MN_IRQ_FIRST - 32);
        }
      else if (irq < (MX8MN_IRQ_FIRST + 96))
        {
  ...
  /* Handle processor exceptions.  Only a few can be disabled */
  ...
  else
    {
      *regaddr = NVIC_SYSHCON;
      if (irq == MX8MN_IRQ_MEMFAULT)
        {
          *bit = NVIC_SYSHCON_MEMFAULTENA;
        }
      else if (irq == MX8MN_IRQ_BUSFAULT)
        {
          *bit = NVIC_SYSHCON_BUSFAULTENA;
        }
      else if (irq == MX8MN_IRQ_USAGEFAULT)
        {
          *bit = NVIC_SYSHCON_USGFAULTENA;
        }
      else if (irq == MX8MN_IRQ_SYSTICK)
        {
...
void up_irqinitialize(void)
{
  ...
  irq_attach(MX8MN_IRQ_SVCALL, arm_svcall, NULL);
  irq_attach(MX8MN_IRQ_HARDFAULT, arm_hardfault, NULL);

  /* Set the priority of the SVCall interrupt */

#ifdef CONFIG_ARCH_IRQPRIO
  /* up_prioritize_irq(MX8MN_IRQ_PENDSV, NVIC_SYSH_PRIORITY_MIN); */
#endif

  mx8mn_prioritize_syscall(NVIC_SYSH_SVCALL_PRIORITY);
  ...
  irq_attach(MX8MN_IRQ_MEMFAULT, arm_memfault, NULL);
  up_enable_irq(MX8MN_IRQ_MEMFAULT);
#endif

  /* Attach all other processor exceptions (except reset and sys tick) */

#ifdef CONFIG_DEBUG_FEATURES
  irq_attach(MX8MN_IRQ_NMI, mx8mn_nmi, NULL);
#ifndef CONFIG_ARM_MPU
  irq_attach(MX8MN_IRQ_MEMFAULT, arm_memfault, NULL);
#endif
  irq_attach(MX8MN_IRQ_BUSFAULT, arm_busfault, NULL);
  irq_attach(MX8MN_IRQ_USAGEFAULT, arm_usagefault, NULL);
  irq_attach(MX8MN_IRQ_PENDSV, mx8mn_pendsv, NULL);
  irq_attach(MX8MN_IRQ_DBGMONITOR, mx8mn_dbgmonitor, NULL);
  irq_attach(MX8MN_IRQ_RESERVED, mx8mn_reserved, NULL);
#endif

  mx8mn_dump_nvic("initial", NR_IRQS);
...
void up_disable_irq(int irq)
{
  ...
  if (mx8mn_irqinfo(irq, &regaddr, &bit, NVIC_CLRENA_OFFSET) == 0)
    {
...
void up_enable_irq(int irq)
{
  ...
  if (mx8mn_irqinfo(irq, &regaddr, &bit, NVIC_ENA_OFFSET) == 0)
    {
...
int up_prioritize_irq(int irq, int priority)
{
  ...
  DEBUGASSERT(irq >= MX8MN_IRQ_MEMFAULT && irq < NR_IRQS &&
  ...
  mx8mn_dump_nvic("prioritize", irq);
  return OK;
}
#endif

/****************************************************************************
 * boards/arm/mx8mn/imx8mn-ddr3l-evk/src/mx8mn_bringup.c
 *
...
int mx8mn_bringup(void)
{
  ...
  ret = mx8mn_i2cdev_initialize();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: mx8mn_i2cdev_initialize() failed: %d\n", ret);
    }
#endif
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: mx8mn_ina219_initialize() failed: %d\n", ret);
    }
#endif

#ifdef CONFIG_MX8MN_SPI_DRIVER
  /* Initialize SPI buses */

  ret = mx8mn_spidev_initialize();
  if (ret < 0)
    {
